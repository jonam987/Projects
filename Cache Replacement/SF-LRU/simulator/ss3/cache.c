	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
 ...
 cache_hit: /* slow hit handler */
  ...
  //}
	if((blk->way_prev) && (cp->policy == LFU))
	{
		SortAlgorithmForLFU(&cp->sets[set]);
		
	}
	if((blk->way_prev) && (cp->policy == SFLRU)){
		AlgorithmForSFLRU(&cp->sets[set],blk);
	}
...
void SortAlgorithmForLFU(struct cache_set_t *set){

	struct cache_blk_t *incoming_blk, *slk;
	unsigned int maximum_val = 0;
	
	incoming_blk = set->way_head;
	while(incoming_blk){
		if(incoming_blk->LFU_count > maximum_val){
			maximum_val = incoming_blk->LFU_count;		
		}
		incoming_blk = incoming_blk->way_next;
	}
	
	slk = set->way_head;
	while(slk)
{
		if(slk->LFU_count == maximum_val){
			update_way_list(set, slk, Head);
			break;		
		}
		slk = slk->way_next;
	}
...
void AlgorithmForSFLRU(struct cache_set_t *set,struct cache_blk_t *blk){
	
	unsigned int count=0, sf_lru = 0;
	struct cache_blk_t *incoming_blk, *slk, *alk;
	int lru_ratio = 100;
	incoming_blk = set->way_head;
	...
	if(blk->MRU_inst != 1) {
		while(incoming_blk){
			if(incoming_blk == blk){
				incoming_blk->MRU_inst = 1;		
			}else{
				incoming_blk->MRU_inst++;
			}
			incoming_blk = incoming_blk->way_next;
			count++;
		}		
	...
	slk = set->way_head;
	//SF-LRU Value caluculation= (count - MRUInst) * LFU_count
	while(slk){
		if((((count - slk->MRU_inst)/lru_ratio) * slk->LFU_count) > sf_lru){
			sf_lru = (((count - slk->MRU_inst)/lru_ratio) * slk->LFU_count);
		}
		slk = slk->way_next;
	}	
	alk = set->way_head;
	while(alk){
		if((((count - alk->MRU_inst)/lru_ratio) * alk->LFU_count) == sf_lru){
			update_way_list(set, alk, Head);
			break;		
		}
		alk = alk->way_next;
	}	
